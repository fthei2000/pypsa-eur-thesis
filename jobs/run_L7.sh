#!/bin/bash
#BSUB -J sector-L7-myopic-2030-2050-cdr
#BSUB -q hpc
#BSUB -n 8
#BSUB -W 48:00
...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L7.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L7.yaml"
if [[ ! -s "$CFG" ]]; then
...
echo "L7 CDR credits run completed successfully."
