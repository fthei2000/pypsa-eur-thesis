...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L5_myopic.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L5-myopic.yaml"
if [[ ! -s "$CFG" ]]; then
...
echo "L5 myopic (2025-2050) run completed successfully."
