...
# In LSF jobs submitted via "bsub < run_L3.sh", BASH_SOURCE points to
...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L3.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L3-short.yaml"
if [[ ! -s "$CFG" ]]; then
  echo "Error: '$CFG' is missing or empty."
  echo "Create/populate it before running L3."
  exit 2
fi
...
echo "L3 short carbon test completed successfully."
