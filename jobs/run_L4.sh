#!/bin/bash
#BSUB -J sector-L4-full-year-c96
#BSUB -q hpc
#BSUB -n 8
#BSUB -W 48:00
...
# In LSF jobs submitted via "bsub < run_L4.sh", BASH_SOURCE points to
...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L4.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L4.yaml"
if [[ ! -s "$CFG" ]]; then
  echo "Error: '$CFG' is missing or empty."
  echo "Create/populate it before running L4."
  exit 2
fi
...
echo "L4 full-year net-zero (c96) run completed successfully."
