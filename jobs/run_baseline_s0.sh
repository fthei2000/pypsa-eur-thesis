...
# In LSF jobs submitted via "bsub < run_baseline_s0.sh", BASH_SOURCE points to
# a temporary file in ~/.lsbatch. Use submit directory instead.
RUN_DIR="${LS_SUBCWD:-$PWD}"
cd "$RUN_DIR"

# Fallback for cases where job is submitted outside the repository.
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_baseline_s0.sh"
    exit 2
  fi
fi

CFG="${CFG:-config/Myruns/supply_curve/config.S0.yaml}"
if [[ ! -s "$CFG" ]]; then
  echo "Error: '$CFG' is missing or empty."
  echo "Set CFG=<path> if you want a different baseline config."
  exit 2
fi
...
if [[ -z "$PIXI_BIN" ]]; then
  ...
  echo "Install Pixi for this user or add its bin directory to PATH."
  exit 127
fi
...
# Redirect cache away from small home directory
...
mkdir -p "$XDG_CACHE_HOME" "$SNAKEMAKE_OUTPUT_CACHE"
...
echo "Baseline s0 run completed successfully."
