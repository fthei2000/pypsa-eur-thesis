#!/bin/bash
#BSUB -J sector-L7-perfect-2030-2050-cdr
#BSUB -q hpc
#BSUB -n 8
#BSUB -W 72:00
...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L7pf.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L7pf.yaml"
if [[ ! -s "$CFG" ]]; then
...
if [[ "$status" -ne 0 ]] && grep -q "Directory cannot be locked" "$tmp_log"; then
  ...
  "$PIXI_BIN" run snakemake make_summary_perfect --unlock --configfile "$CFG"
  run_snakemake
elif [[ "$status" -ne 0 ]]; then
  exit "$status"
fi

echo "L7 perfect foresight CDR credits run completed successfully."