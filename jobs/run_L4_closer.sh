#!/bin/bash
#BSUB -J sector-L4-closer
#BSUB -q hpc
#BSUB -n 8
#BSUB -W 36:00
#BSUB -R "rusage[mem=22000]"
#BSUB -o logs/%J.out
#BSUB -e logs/%J.err
...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L4_closer.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L4-closer.yaml"
if [[ ! -s "$CFG" ]]; then
...
echo "L4-closer run completed successfully."
