...
if [[ ! -f "pixi.toml" ]]; then
  ...
  if [[ -f "$FALLBACK_DIR/pixi.toml" ]]; then
    ...
    echo "Submit from the repository root, e.g.: cd /work3/s240459/pypsa-eur-thesis && bsub < run_L4_fast.sh"
    exit 2
  fi
fi

CFG="config/Myruns/config.L4-fast.yaml"
if [[ ! -s "$CFG" ]]; then
...
echo "L4-fast run completed successfully."
